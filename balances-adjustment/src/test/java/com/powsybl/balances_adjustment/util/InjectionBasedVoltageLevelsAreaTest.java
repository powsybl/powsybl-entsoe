 ...
 */
class InjectionBasedVoltageLevelsAreaTest {
    private Network testNetwork;
    private VoltageLevelsAreaFactory voltageLevelsArea;

    @BeforeEach
    void setUp() {
        testNetwork = Network.read(
            "testCase.xiidm",
            BorderBasedVoltageLevelsAreaTest.class.getResourceAsStream("/testCase.xiidm")
        );
    ...
    void testGetNetPosition() {
        ...
        assertEquals(
            flows.stream().mapToDouble(f -> f).sum(),
            voltageLevelsArea.create(testNetwork, true).getNetPosition(), 1e-3
        );
    }

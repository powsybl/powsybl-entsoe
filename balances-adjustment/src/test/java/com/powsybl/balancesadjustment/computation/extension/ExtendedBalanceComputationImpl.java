...
import com.powsybl.balancesadjustment.computation.BalanceComputationImpl;
...
class ExtendedBalanceComputationImpl extends BalanceComputationImpl {
    ...
    ExtendedBalanceComputationImpl(List<BalanceComputationArea> areas, ComputationManager computationManager, LoadFlow.Runner loadFlowRunner) {
    ...
    protected double computeTotalMismatch(BalanceComputationRunningContext context) {
        context.getBalanceMismatches()
            .forEach(
                (area, mismatch) -> LOGGER.info("{} area mismatch is {} (thresholdNetPosition is {})", area.getName(), mismatch, context.getParameters().getThresholdNetPosition())
            );
